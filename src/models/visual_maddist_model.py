...
class VisualMadDistModel(Model):
    """The model used to train the MadDist objective on pixel-based AntMaze observations."""


    identifier = 'visual_maddist'
    aliases = ['visual_maddist_model', 'VisualMadDistModel', 'VisualMadDist', 'visual_mad']
    ...
    def _construct_model(self, input_dim: int, hidden_dims: list[int] = [512, 512, 256], latent_dim: int = 512) -> nn.Module:
        ...
        layers += [mad_block(256*4*4, hidden_dims[0])]
